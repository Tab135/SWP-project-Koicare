...
const UpdateNews = () => {
  ...
  useEffect(() => {
    ...
    const fetchNewsData = async () => {
      try {
        const response = await fetch(`http://localhost:8080/public/news/9`);
        ...
        const imageResponse = await fetch(`http://localhost:8080/public/news/9/image`);
        if (imageResponse.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8080/shop/news/9/update`, {
        method: 'PUT',
        headers: {
